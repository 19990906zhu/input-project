...
def run_cv(X, y):
    ...
    for fold_id, (train_idx, val_idx) in enumerate(cv.split(X, y), start=1):
        ...
        # FIXED: fit scaler on the training fold only, then transform the validation fold.
        if USE_SCALER:
            scaler = StandardScaler()
            X_train_model = pd.DataFrame(scaler.fit_transform(X_train), columns=X.columns, index=X_train.index)
            X_val_model = pd.DataFrame(scaler.transform(X_val), columns=X.columns, index=X_val.index)
        else:
        ...
        train_prob = get_positive_probability(model, X_train_model)
        val_prob = get_positive_probability(model, X_val_model)
...
def run_rfecv_and_plot_top10(X, y, output_dir):
    ...
    if USE_SCALER:
        estimator = SkPipeline([("scaler", StandardScaler()), ("model", build_model())])
    ...
    final_model.fit(X_selected_model, y)
