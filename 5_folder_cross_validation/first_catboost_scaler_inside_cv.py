...
SAMPLING_RATIOS = [0.5, 0.51, 0.52, 0.53, 0.54, 0.55, 0.56, 0.57, 0.58, 0.59, 0.6, 0.61, 0.62, 0.63, 0.64, 0.65, 0.66, 0.67, 0.68, 0.69, 0.7]
...
def run_catboost_shap(X, y, best_sampler_name, best_sampler_class, best_ratio, output_dir):
    ...
    X_model = fit_transform_full_training_if_needed(X, shap_dir, prefix="catboost_shap_full_training")
    sampler = make_sampler(best_sampler_name, best_sampler_class, best_ratio, y)
    X_res, y_res = sampler.fit_resample(X_model, y)
    ...
    shap_df = pd.DataFrame(shap_values_to_use, columns=X_model.columns)
    ...
    shap.summary_plot(shap_values_to_use, X_model, show=False, max_display=min(20, X_model.shape[1]))
    plt.tight_layout()
