...
MODEL_SHORT = "AdaBoost"
OUTPUT_BASE_DIR = r"D:\萝卜投稿专用\Original Research - Spinal Infection Following Internal Fixation Surgery\插值选择\Xgboost\lr=0.1\建模-1_adaboost-booststraps"
...
# Original AdaBoost setting used 0.66.
# Add more ratios if needed, for example: [0.60, 0.66, 0.70]
SAMPLING_RATIOS = [0.62]
...
def create_timestamped_output_dir(base_dir):
    ...
    out_dir = os.path.join(base_dir, f"AdaBoost_FINAL_{stamp}")
...
def run_one_setting(X, y, sampler_name, sampler_class, ratio):
    ...
    return {
        ...
        "Eligible": (TARGET_VAL_AUPRC_LOW <= val_auprc <= TARGET_VAL_AUPRC_HIGH) and ((train_auprc - val_auprc) <= MAX_GAP),
        "Threshold": THRESHOLD,
...
def plot_combined_pr_roc(best_obj, output_dir):
    fig_dir = os.path.join(output_dir, "PR_ROC_curves_FINAL")
    ensure_output_dir(fig_dir)

    sampler_name = best_obj["SMOTE_Method"]
    ratio_text = ratio_to_text(best_obj["Sampling_Ratio"])

    train_auprc_mean = float(best_obj["Train_AUPRC"])
    val_auprc_mean = float(best_obj["Val_AUPRC"])
    train_auc_mean = get_mean_metric(best_obj["Summary_DF"], "Train", "AUC")
    val_auc_mean = get_mean_metric(best_obj["Summary_DF"], "Validation", "AUC")

    train_y, train_prob = best_obj["train_y"], best_obj["train_prob"]
    val_y, val_prob = best_obj["val_y"], best_obj["val_prob"]

    train_pooled_auprc = average_precision_score(train_y, train_prob)
    val_pooled_auprc = average_precision_score(val_y, val_prob)
    train_pooled_auc = roc_auc_score(train_y, train_prob)
    val_pooled_auc = roc_auc_score(val_y, val_prob)

    check_df = pd.DataFrame([{
        "Best_SMOTE_Method": sampler_name,
        "Best_Sampling_Ratio": best_obj["Sampling_Ratio"],
        "Train_AUPRC_for_PR_legend_5fold_mean": train_auprc_mean,
        "Validation_AUPRC_for_PR_legend_5fold_mean": val_auprc_mean,
        "Train_AUPRC_pooled_curve_area": train_pooled_auprc,
        "Validation_AUPRC_pooled_curve_area": val_pooled_auprc,
        "Train_AUC_for_ROC_legend_5fold_mean": train_auc_mean,
        "Validation_AUC_for_ROC_legend_5fold_mean": val_auc_mean,
        "Train_AUC_pooled_curve_area": train_pooled_auc,
        "Validation_AUC_pooled_curve_area": val_pooled_auc,
        "Threshold": THRESHOLD,
        "Note": "Figure legend uses 5-fold mean values from the selected best setting. Curve shape uses pooled fold predictions.",
    }])
    ...
    sci_axes(ax)
    fig.tight_layout()
    pr_name = f"OPEN_THIS_PR_curve_BEST_{sampler_name}_ratio_{ratio_text}_ValAUPRC_{val_auprc_mean:.3f}".replace(".", "p")
    ...
    sci_axes(ax)
    fig.tight_layout()
    roc_name = f"OPEN_THIS_ROC_curve_BEST_{sampler_name}_ratio_{ratio_text}_ValAUC_{val_auc_mean:.3f}".replace(".", "p")
    save_figure(fig, fig_dir, roc_name)


def bootstrap_fold_metrics(fold_df, output_dir, best_model_name=MODEL_SHORT):
    """Bootstrap 95% CI from the 5-fold metric values after model selection."""
...
# =============================================================================
# 4. RFECV + feature importance after RFECV
# =============================================================================
def run_rfecv_and_plot_top10(X, y, best_sampler_name, best_sampler_class, best_ratio, output_dir):
    ...
    # Correct figure: RFECV selects features first, then AdaBoost feature importance is recalculated only on selected features.
    X_selected = X[selected_features]
    X_selected_model = fit_transform_full_training_if_needed(X_selected, rfecv_dir, prefix="rfecv_selected_full_training")
    final_sampler = make_sampler(best_sampler_name, best_sampler_class, best_ratio, y)
...
def main():
    ...
    warn_if_prezscore_input(resolved_file)
    ...
    plot_combined_pr_roc(best_obj, output_dir)
