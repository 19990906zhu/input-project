...
MODEL_SHORT = "DecisionTree"

# Root output folder. A timestamped subfolder will be automatically created inside it.
OUTPUT_ROOT = r"D:\萝卜投稿专用\Original Research - Spinal Infection Following Internal Fixation Surgery\插值选择\Xgboost\lr=0.1\建模-1_decision_tree_FINAL_CLEAN"

USE_SCALER = False
SMOTE_K_NEIGHBORS = 5

# 你之前 Decision Tree 截图里最佳是 SMOTE ratio=0.70；如果要继续比较多个比例，可写成 [0.65, 0.70, 0.75]
SAMPLING_RATIOS = [0.50]
...
# 如果只想固定跑普通 SMOTE，把下面只保留 "SMOTE": SMOTE 即可。
SAMPLER_CONFIGS = {
    "SMOTE": SMOTE,
...
def make_timestamped_output_dir():
    ...
    out_dir = os.path.join(OUTPUT_ROOT, f"DecisionTree_FINAL_CLEAN_{timestamp}")
...
def plot_combined_pr_roc(best_obj, best_row, best_summary, output_dir):
    ...
    check_df = pd.DataFrame([{
        ...
        "Threshold": THRESHOLD,
        "Note": "Legend values are copied from 01_all_results.xlsx best row; curve shape uses pooled CV predictions.",
    }])
...
# =============================================================================
# 4. Feature importance for best Decision Tree model
# =============================================================================
def run_decision_tree_feature_importance(X, y, best_sampler_name, best_sampler_class, best_ratio, output_dir):
    imp_dir = os.path.join(output_dir, "Feature_Importance_DecisionTree")
    ensure_output_dir(imp_dir)
    ...
    imp_df = pd.DataFrame({
        "Feature": X.columns,
        "DecisionTree_Importance": imp_values,
    }).sort_values("DecisionTree_Importance", ascending=False).reset_index(drop=True)
    save_excel_formatted(imp_df, os.path.join(imp_dir, "DecisionTree_feature_importance.xlsx"), "Importance")

    plot_df = imp_df.head(TOP_N_FEATURES).sort_values("DecisionTree_Importance", ascending=True)
    height = max(4.5, 0.38 * len(plot_df))
    fig, ax = plt.subplots(figsize=(8, height), dpi=600)
    ax.barh(plot_df["Feature"], plot_df["DecisionTree_Importance"], color=COLOR_BAR, edgecolor="black", linewidth=0.4)
    ax.set_xlabel("Decision Tree feature importance")
    ax.set_ylabel("")
    ax.set_title(f"Top {min(TOP_N_FEATURES, len(plot_df))} Decision Tree Features")
    sci_axes(ax)
    fig.tight_layout()
    save_figure(fig, imp_dir, "DecisionTree_feature_importance_top10")
...
def main():
    ...
    print("=" * 80)
    print("RUNNING CLEAN DECISION TREE FINAL SCRIPT")
    ...
    # Core Excel outputs: same structure as CatBoost version.
    ...
    run_decision_tree_feature_importance(X, y, best_sampler_name, best_sampler_class, best_ratio, OUTPUT_DIR)
